...
##################################################################################
#    HOTELDRUID
#    Copyright (C) 2001-2022 by Marco Maria Francesco De Santis (marco@digitaldruid.net)
#
...
$risul = @esegui_query("SET default_storage_engine=MYISAM",1);
if (!$risul) {
sleep(1);
@esegui_query("SET default_storage_engine=MYISAM",1);
} # fine if (!$risul)
} # fine if ($PHPR_DB_TYPE == "mysql" or $PHPR_DB_TYPE == "mysqli")
...
$aggiornato = "SI";
#
# Ricordarsi nelle versioni successive a 3.01 di passare qui db mysql a utf8mb4 se non lo è (come in creadb, funzioni_anno e backup)
#
for ($num1 = 0 ; $num1 < $num_anni ; $num1++) {
$anno_mostra = risul_query($anni,$num1,'idanni');
$tablecosti = $PHPR_TAB_PRE."costi".$anno_mostra;
$tablecosti_temp = $PHPR_TAB_PRE."cost".$anno_mostra;
esegui_query("alter table $tablecosti add column valuta text ");
esegui_query("alter table $tablecosti add column costo_valuta float8 ");
esegui_query("alter table $tablecosti rename to $tablecosti_temp ");
esegui_query("create table $tablecosti (idcosti integer unique,nome_costo text, val_costo float8, tipo_costo text, valuta text, costo_valuta float8, nome_cassa varchar(70), persona_costo text, provenienza_costo text, metodo_pagamento text, datainserimento $DATETIME, hostinserimento varchar(50), utente_inserimento integer) ");
esegui_query("insert into $tablecosti select idcosti,nome_costo,val_costo,tipo_costo,valuta,costo_valuta,nome_cassa,persona_costo,provenienza_costo,metodo_pagamento,datainserimento,hostinserimento,utente_inserimento from $tablecosti_temp ");
esegui_query("drop table $tablecosti_temp ");
$tablesoldi = $PHPR_TAB_PRE."soldi".$anno_mostra;
$tablesoldi_temp = $PHPR_TAB_PRE."sold".$anno_mostra;
esegui_query("alter table $tablesoldi add column valuta text ");
esegui_query("alter table $tablesoldi add column saldo_valuta float8 ");
esegui_query("alter table $tablesoldi rename to $tablesoldi_temp ");
esegui_query("create table $tablesoldi (idsoldi integer unique, motivazione text, id_pagamento text, metodo_pagamento text, saldo_prenota float8, saldo_cassa float8, soldi_prima float8, valuta text, saldo_valuta float8, data_inserimento $DATETIME, utente_inserimento integer) ");
esegui_query("insert into $tablesoldi select idsoldi,motivazione,id_pagamento,metodo_pagamento,saldo_prenota,saldo_cassa,soldi_prima,valuta,saldo_valuta,data_inserimento,utente_inserimento from $tablesoldi_temp ");
esegui_query("drop table $tablesoldi_temp ");
$tableprenota = $PHPR_TAB_PRE."prenota".$anno_mostra;
$tableprenota_temp = $PHPR_TAB_PRE."pren".$anno_mostra;
esegui_query("alter table $tableprenota add column valuta text ");
esegui_query("alter table $tableprenota rename to $tableprenota_temp ");
...
esegui_query("insert into $tableprenota select idprenota,idclienti,idappartamenti,iddatainizio,iddatafine,assegnazioneapp,app_assegnabili,num_persone,cat_persone,idprenota_compagna,tariffa,tariffesettimanali,incompatibilita,sconto,tariffa_tot,caparra,commissioni,tasseperc,pagato,valuta,metodo_pagamento,codice,origine,commento,conferma,checkin,checkout,id_anni_prec,datainserimento,hostinserimento,data_modifica,utente_inserimento from $tableprenota_temp ");
esegui_query("drop table $tableprenota_temp ");
$tableprenotacanc = $PHPR_TAB_PRE."prenotacanc".$anno_mostra;
esegui_query("create table $tableprenotacanc (idprenota integer primary key, idclienti integer, idappartamenti varchar(100), iddatainizio integer, iddatafine integer, assegnazioneapp varchar(4), app_assegnabili text, num_persone integer, cat_persone text, idprenota_compagna text, tariffa text, tariffesettimanali text, incompatibilita text, sconto float8, tariffa_tot float8, caparra float8, commissioni float8, tasseperc float4, pagato float8, valuta text, metodo_pagamento text, codice varchar(10), origine varchar(70), commento text, conferma varchar(4), checkin $DATETIME, checkout $DATETIME, id_anni_prec text, datainserimento $DATETIME, hostinserimento varchar(50), data_modifica $DATETIME, utente_inserimento integer )");
} # fine for $num1
$tableclienti = $PHPR_TAB_PRE."clienti";
$tableclienti_temp = $PHPR_TAB_PRE."clien";
esegui_query("alter table $tableclienti add column email2 text ");
esegui_query("alter table $tableclienti add column email3 text ");
esegui_query("alter table $tableclienti rename to $tableclienti_temp ");
...
esegui_query("insert into $tableclienti select idclienti,cognome,nome,soprannome,sesso,titolo,lingua,datanascita,cittanascita,regionenascita,nazionenascita,documento,scadenzadoc,tipodoc,cittadoc,regionedoc,nazionedoc,nazionalita,nazione,regione,citta,via,numcivico,cap,telefono,telefono2,telefono3,fax,email,email2,email3,cod_fiscale,partita_iva,commento,max_num_ordine,idclienti_compagni,doc_inviati,datainserimento,hostinserimento,utente_inserimento from $tableclienti_temp ");
esegui_query("drop table $tableclienti_temp ");
#
# Ricordarsi nelle versioni successive a 3.01 di passare qui il db mysql a character set originale se lo si è cambiato
#
$priv_glob = esegui_query("select * from ".$PHPR_TAB_PRE."privilegi where anno = '1' ");
for ($num1 = 0 ; $num1 < numlin_query($priv_glob) ; $num1++) {
$id_ut = risul_query($priv_glob,$num1,'idutente');
$priv_mod_pers = risul_query($priv_glob,$num1,'priv_mod_pers');
esegui_query("update ".$PHPR_TAB_PRE."privilegi set priv_mod_pers = '$priv_mod_pers"."sn' where anno = '1' and idutente = '$id_ut' ");
} # fine for $num1
$privilegi_anni = esegui_query("select * from ".$PHPR_TAB_PRE."privilegi where anno != '1' ");
for ($num1 = 0 ; $num1 < numlin_query($privilegi_anni) ; $num1++) {
$anno_priv =  risul_query($privilegi_anni,$num1,'anno');
$idutente_priv =  risul_query($privilegi_anni,$num1,'idutente');
$priv_mod_prec = risul_query($privilegi_anni,$num1,'priv_mod_prenota');
$priv_ins_tar_prec = risul_query($privilegi_anni,$num1,'priv_ins_tariffe');
esegui_query("update ".$PHPR_TAB_PRE."privilegi set priv_mod_prenota = '".$priv_mod_prec."s', priv_ins_tariffe = '".$priv_ins_tar_prec."nn' where idutente = '$idutente_priv' and anno = '$anno_priv' ");
} # fine for $num1
} # fine if ($versione_corrente < "3.00")

if ($versione_corrente < "3.01") {
$aggiornato = "SI";
if ($PHPR_DB_TYPE == "mysqli") {
$disp_err_orig = ini_get('display_errors');
if ($disp_err_orig) ini_set('display_errors','0');
#esegui_query("ALTER DATABASE `$PHPR_DB_NAME` CHARACTER SET = utf8mb4 COLLATE = utf8mb4_unicode_520_ci");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."anni` CONVERT TO CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."appartamenti` CONVERT TO CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."beniinventario` CONVERT TO CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."cache` CONVERT TO CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."casse` CONVERT TO CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."citta` CONVERT TO CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."clienti` CONVERT TO CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."contratti` CONVERT TO CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."descrizioni` CONVERT TO CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."documentiid` CONVERT TO CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."gruppi` CONVERT TO CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."interconnessioni` CONVERT TO CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."magazzini` CONVERT TO CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."messaggi` CONVERT TO CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."nazioni` CONVERT TO CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."parentele` CONVERT TO CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."personalizza` CONVERT TO CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."privilegi` CONVERT TO CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."regioni` CONVERT TO CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."relclienti` CONVERT TO CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."relgruppi` CONVERT TO CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."relinventario` CONVERT TO CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."relutenti` CONVERT TO CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."sessioni` CONVERT TO CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."transazioni` CONVERT TO CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."transazioniweb` CONVERT TO CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."utenti` CONVERT TO CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."versioni` CONVERT TO CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."transazioni` CHANGE `idtransazioni` `idtransazioni` varchar(30) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NOT NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."transazioni` CHANGE `idsessione` `idsessione` varchar(30) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."transazioni` CHANGE `tipo_transazione` `tipo_transazione` varchar(5) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."relclienti` CHANGE `tipo` `tipo` varchar(12) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."relclienti` CHANGE `hostinserimento` `hostinserimento` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."messaggi` CHANGE `tipo_messaggio` `tipo_messaggio` varchar(8) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."messaggi` CHANGE `stato` `stato` varchar(8) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."beniinventario` CHANGE `nome_bene` `nome_bene` varchar(70) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."beniinventario` CHANGE `codice_bene` `codice_bene` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."beniinventario` CHANGE `hostinserimento` `hostinserimento` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."personalizza` CHANGE `idpersonalizza` `idpersonalizza` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NOT NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."regioni` CHANGE `nome_regione` `nome_regione` varchar(70) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."regioni` CHANGE `codice_regione` `codice_regione` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."regioni` CHANGE `codice2_regione` `codice2_regione` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."regioni` CHANGE `codice3_regione` `codice3_regione` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."regioni` CHANGE `hostinserimento` `hostinserimento` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."descrizioni` CHANGE `tipo` `tipo` varchar(16) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."descrizioni` CHANGE `lingua` `lingua` varchar(3) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."parentele` CHANGE `nome_parentela` `nome_parentela` varchar(70) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."parentele` CHANGE `codice_parentela` `codice_parentela` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."parentele` CHANGE `codice2_parentela` `codice2_parentela` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."parentele` CHANGE `codice3_parentela` `codice3_parentela` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."parentele` CHANGE `hostinserimento` `hostinserimento` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."relgruppi` CHANGE `hostinserimento` `hostinserimento` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."casse` CHANGE `nome_cassa` `nome_cassa` varchar(70) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."casse` CHANGE `stato` `stato` varchar(8) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."casse` CHANGE `codice_cassa` `codice_cassa` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."casse` CHANGE `hostinserimento` `hostinserimento` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."nazioni` CHANGE `nome_nazione` `nome_nazione` varchar(70) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."nazioni` CHANGE `codice_nazione` `codice_nazione` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."nazioni` CHANGE `codice2_nazione` `codice2_nazione` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."nazioni` CHANGE `codice3_nazione` `codice3_nazione` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."nazioni` CHANGE `hostinserimento` `hostinserimento` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."utenti` CHANGE `tipo_pass` `tipo_pass` varchar(1) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."utenti` CHANGE `hostinserimento` `hostinserimento` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."transazioniweb` CHANGE `idtransazioni` `idtransazioni` varchar(30) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NOT NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."transazioniweb` CHANGE `idsessione` `idsessione` varchar(30) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."transazioniweb` CHANGE `tipo_transazione` `tipo_transazione` varchar(5) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."sessioni` CHANGE `idsessioni` `idsessioni` varchar(30) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NOT NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."sessioni` CHANGE `tipo_conn` `tipo_conn` varchar(12) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."magazzini` CHANGE `nome_magazzino` `nome_magazzino` varchar(70) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."magazzini` CHANGE `codice_magazzino` `codice_magazzino` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."magazzini` CHANGE `hostinserimento` `hostinserimento` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."privilegi` CHANGE `cassa_pagamenti` `cassa_pagamenti` varchar(70) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."privilegi` CHANGE `priv_ins_prenota` `priv_ins_prenota` varchar(20) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."privilegi` CHANGE `priv_mod_prenota` `priv_mod_prenota` varchar(35) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."privilegi` CHANGE `priv_mod_pers` `priv_mod_pers` varchar(15) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."privilegi` CHANGE `priv_ins_clienti` `priv_ins_clienti` varchar(5) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."privilegi` CHANGE `priv_ins_costi` `priv_ins_costi` varchar(10) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."privilegi` CHANGE `priv_vedi_tab` `priv_vedi_tab` varchar(30) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."privilegi` CHANGE `priv_ins_tariffe` `priv_ins_tariffe` varchar(10) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."privilegi` CHANGE `priv_ins_regole` `priv_ins_regole` varchar(10) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."privilegi` CHANGE `priv_messaggi` `priv_messaggi` varchar(10) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."privilegi` CHANGE `priv_inventario` `priv_inventario` varchar(10) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."clienti` CHANGE `cognome` `cognome` varchar(70) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NOT NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."clienti` CHANGE `nome` `nome` varchar(70) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."clienti` CHANGE `soprannome` `soprannome` varchar(70) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."clienti` CHANGE `sesso` `sesso` char(1) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."clienti` CHANGE `titolo` `titolo` varchar(30) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."clienti` CHANGE `lingua` `lingua` varchar(14) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."clienti` CHANGE `cittanascita` `cittanascita` varchar(70) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."clienti` CHANGE `regionenascita` `regionenascita` varchar(70) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."clienti` CHANGE `nazionenascita` `nazionenascita` varchar(70) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."clienti` CHANGE `documento` `documento` varchar(70) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."clienti` CHANGE `tipodoc` `tipodoc` varchar(70) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."clienti` CHANGE `cittadoc` `cittadoc` varchar(70) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."clienti` CHANGE `regionedoc` `regionedoc` varchar(70) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."clienti` CHANGE `nazionedoc` `nazionedoc` varchar(70) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."clienti` CHANGE `nazionalita` `nazionalita` varchar(70) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."clienti` CHANGE `nazione` `nazione` varchar(70) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."clienti` CHANGE `regione` `regione` varchar(70) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."clienti` CHANGE `citta` `citta` varchar(70) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."clienti` CHANGE `via` `via` varchar(70) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."clienti` CHANGE `numcivico` `numcivico` varchar(30) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."clienti` CHANGE `cap` `cap` varchar(30) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."clienti` CHANGE `telefono` `telefono` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."clienti` CHANGE `telefono2` `telefono2` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."clienti` CHANGE `telefono3` `telefono3` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."clienti` CHANGE `fax` `fax` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."clienti` CHANGE `cod_fiscale` `cod_fiscale` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."clienti` CHANGE `partita_iva` `partita_iva` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."clienti` CHANGE `hostinserimento` `hostinserimento` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."documentiid` CHANGE `nome_documentoid` `nome_documentoid` varchar(70) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."documentiid` CHANGE `codice_documentoid` `codice_documentoid` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."documentiid` CHANGE `codice2_documentoid` `codice2_documentoid` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."documentiid` CHANGE `codice3_documentoid` `codice3_documentoid` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."documentiid` CHANGE `hostinserimento` `hostinserimento` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."cache` CHANGE `tipo` `tipo` varchar(8) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."appartamenti` CHANGE `idappartamenti` `idappartamenti` varchar(100) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NOT NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."appartamenti` CHANGE `letto` `letto` varchar(1) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."relutenti` CHANGE `hostinserimento` `hostinserimento` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."interconnessioni` CHANGE `tipoid` `tipoid` varchar(12) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."interconnessioni` CHANGE `nome_ic` `nome_ic` varchar(24) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."interconnessioni` CHANGE `hostinserimento` `hostinserimento` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."citta` CHANGE `nome_citta` `nome_citta` varchar(70) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."citta` CHANGE `codice_citta` `codice_citta` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."citta` CHANGE `codice2_citta` `codice2_citta` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."citta` CHANGE `codice3_citta` `codice3_citta` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."citta` CHANGE `hostinserimento` `hostinserimento` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."contratti` CHANGE `tipo` `tipo` varchar(8) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."contratti` CHANGE `testo` `testo` mediumtext CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."relinventario` CHANGE `idappartamento` `idappartamento` varchar(100) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."relinventario` CHANGE `richiesto_checkin` `richiesto_checkin` varchar(2) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."relinventario` CHANGE `hostinserimento` `hostinserimento` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."transazioni` CHANGE `spostamenti` `spostamenti` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."transazioni` CHANGE `dati_transazione1` `dati_transazione1` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."transazioni` CHANGE `dati_transazione2` `dati_transazione2` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."transazioni` CHANGE `dati_transazione3` `dati_transazione3` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."transazioni` CHANGE `dati_transazione4` `dati_transazione4` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."transazioni` CHANGE `dati_transazione5` `dati_transazione5` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."transazioni` CHANGE `dati_transazione6` `dati_transazione6` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."transazioni` CHANGE `dati_transazione7` `dati_transazione7` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."transazioni` CHANGE `dati_transazione8` `dati_transazione8` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."transazioni` CHANGE `dati_transazione9` `dati_transazione9` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."transazioni` CHANGE `dati_transazione10` `dati_transazione10` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."transazioni` CHANGE `dati_transazione11` `dati_transazione11` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."transazioni` CHANGE `dati_transazione12` `dati_transazione12` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."transazioni` CHANGE `dati_transazione13` `dati_transazione13` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."transazioni` CHANGE `dati_transazione14` `dati_transazione14` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."transazioni` CHANGE `dati_transazione15` `dati_transazione15` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."transazioni` CHANGE `dati_transazione16` `dati_transazione16` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."transazioni` CHANGE `dati_transazione17` `dati_transazione17` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."transazioni` CHANGE `dati_transazione18` `dati_transazione18` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."transazioni` CHANGE `dati_transazione19` `dati_transazione19` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."transazioni` CHANGE `dati_transazione20` `dati_transazione20` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."transazioni` CHANGE `dati_transazione21` `dati_transazione21` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."transazioni` CHANGE `dati_transazione22` `dati_transazione22` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."relclienti` CHANGE `testo1` `testo1` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."relclienti` CHANGE `testo2` `testo2` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."relclienti` CHANGE `testo3` `testo3` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."relclienti` CHANGE `testo4` `testo4` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."relclienti` CHANGE `testo5` `testo5` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."relclienti` CHANGE `testo6` `testo6` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."relclienti` CHANGE `testo7` `testo7` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."relclienti` CHANGE `testo8` `testo8` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."messaggi` CHANGE `idutenti` `idutenti` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."messaggi` CHANGE `idutenti_visto` `idutenti_visto` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."messaggi` CHANGE `mittente` `mittente` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."messaggi` CHANGE `testo` `testo` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."messaggi` CHANGE `dati_messaggio1` `dati_messaggio1` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."messaggi` CHANGE `dati_messaggio2` `dati_messaggio2` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."messaggi` CHANGE `dati_messaggio3` `dati_messaggio3` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."messaggi` CHANGE `dati_messaggio4` `dati_messaggio4` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."messaggi` CHANGE `dati_messaggio5` `dati_messaggio5` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."messaggi` CHANGE `dati_messaggio6` `dati_messaggio6` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."messaggi` CHANGE `dati_messaggio7` `dati_messaggio7` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."messaggi` CHANGE `dati_messaggio8` `dati_messaggio8` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."messaggi` CHANGE `dati_messaggio9` `dati_messaggio9` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."messaggi` CHANGE `dati_messaggio10` `dati_messaggio10` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."messaggi` CHANGE `dati_messaggio11` `dati_messaggio11` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."messaggi` CHANGE `dati_messaggio12` `dati_messaggio12` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."messaggi` CHANGE `dati_messaggio13` `dati_messaggio13` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."messaggi` CHANGE `dati_messaggio14` `dati_messaggio14` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."messaggi` CHANGE `dati_messaggio15` `dati_messaggio15` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."messaggi` CHANGE `dati_messaggio16` `dati_messaggio16` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."messaggi` CHANGE `dati_messaggio17` `dati_messaggio17` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."messaggi` CHANGE `dati_messaggio18` `dati_messaggio18` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."messaggi` CHANGE `dati_messaggio19` `dati_messaggio19` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."messaggi` CHANGE `dati_messaggio20` `dati_messaggio20` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."messaggi` CHANGE `dati_messaggio21` `dati_messaggio21` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."messaggi` CHANGE `dati_messaggio22` `dati_messaggio22` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."beniinventario` CHANGE `descrizione_bene` `descrizione_bene` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."personalizza` CHANGE `valpersonalizza` `valpersonalizza` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."descrizioni` CHANGE `nome` `nome` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NOT NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."descrizioni` CHANGE `testo` `testo` mediumtext CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."gruppi` CHANGE `nome_gruppo` `nome_gruppo` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."casse` CHANGE `descrizione_cassa` `descrizione_cassa` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."utenti` CHANGE `nome_utente` `nome_utente` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."utenti` CHANGE `password` `password` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."utenti` CHANGE `salt` `salt` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."transazioniweb` CHANGE `spostamenti` `spostamenti` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."transazioniweb` CHANGE `dati_transazione1` `dati_transazione1` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."transazioniweb` CHANGE `dati_transazione2` `dati_transazione2` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."transazioniweb` CHANGE `dati_transazione3` `dati_transazione3` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."transazioniweb` CHANGE `dati_transazione4` `dati_transazione4` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."transazioniweb` CHANGE `dati_transazione5` `dati_transazione5` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."transazioniweb` CHANGE `dati_transazione6` `dati_transazione6` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."transazioniweb` CHANGE `dati_transazione7` `dati_transazione7` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."transazioniweb` CHANGE `dati_transazione8` `dati_transazione8` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."transazioniweb` CHANGE `dati_transazione9` `dati_transazione9` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."transazioniweb` CHANGE `dati_transazione10` `dati_transazione10` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."transazioniweb` CHANGE `dati_transazione11` `dati_transazione11` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."transazioniweb` CHANGE `dati_transazione12` `dati_transazione12` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."transazioniweb` CHANGE `dati_transazione13` `dati_transazione13` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."transazioniweb` CHANGE `dati_transazione14` `dati_transazione14` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."transazioniweb` CHANGE `dati_transazione15` `dati_transazione15` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."transazioniweb` CHANGE `dati_transazione16` `dati_transazione16` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."transazioniweb` CHANGE `dati_transazione17` `dati_transazione17` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."transazioniweb` CHANGE `dati_transazione18` `dati_transazione18` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."transazioniweb` CHANGE `dati_transazione19` `dati_transazione19` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."transazioniweb` CHANGE `dati_transazione20` `dati_transazione20` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."transazioniweb` CHANGE `dati_transazione21` `dati_transazione21` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."transazioniweb` CHANGE `dati_transazione22` `dati_transazione22` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."sessioni` CHANGE `indirizzo_ip` `indirizzo_ip` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."sessioni` CHANGE `user_agent` `user_agent` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."magazzini` CHANGE `descrizione_magazzino` `descrizione_magazzino` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."magazzini` CHANGE `numpiano` `numpiano` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."magazzini` CHANGE `numcasa` `numcasa` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."privilegi` CHANGE `regole1_consentite` `regole1_consentite` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."privilegi` CHANGE `tariffe_consentite` `tariffe_consentite` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."privilegi` CHANGE `costi_agg_consentiti` `costi_agg_consentiti` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."privilegi` CHANGE `contratti_consentiti` `contratti_consentiti` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."privilegi` CHANGE `casse_consentite` `casse_consentite` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."privilegi` CHANGE `prefisso_clienti` `prefisso_clienti` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."clienti` CHANGE `email` `email` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."clienti` CHANGE `email2` `email2` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."clienti` CHANGE `email3` `email3` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."clienti` CHANGE `commento` `commento` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."clienti` CHANGE `idclienti_compagni` `idclienti_compagni` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."clienti` CHANGE `doc_inviati` `doc_inviati` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."cache` CHANGE `testo` `testo` mediumtext CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."appartamenti` CHANGE `numpiano` `numpiano` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."appartamenti` CHANGE `numcasa` `numcasa` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."appartamenti` CHANGE `app_vicini` `app_vicini` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."appartamenti` CHANGE `commento` `commento` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."anni` CHANGE `tipo_periodi` `tipo_periodi` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."interconnessioni` CHANGE `idremoto1` `idremoto1` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."interconnessioni` CHANGE `idremoto2` `idremoto2` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."anni` ENGINE = MyISAM");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."appartamenti` ENGINE = MyISAM");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."beniinventario` ENGINE = MyISAM");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."cache` ENGINE = MyISAM");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."casse` ENGINE = MyISAM");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."citta` ENGINE = MyISAM");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."clienti` ENGINE = MyISAM");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."contratti` ENGINE = MyISAM");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."descrizioni` ENGINE = MyISAM");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."documentiid` ENGINE = MyISAM");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."gruppi` ENGINE = MyISAM");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."interconnessioni` ENGINE = MyISAM");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."magazzini` ENGINE = MyISAM");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."messaggi` ENGINE = MyISAM");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."nazioni` ENGINE = MyISAM");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."parentele` ENGINE = MyISAM");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."personalizza` ENGINE = MyISAM");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."privilegi` ENGINE = MyISAM");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."regioni` ENGINE = MyISAM");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."relclienti` ENGINE = MyISAM");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."relgruppi` ENGINE = MyISAM");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."relinventario` ENGINE = MyISAM");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."relutenti` ENGINE = MyISAM");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."sessioni` ENGINE = MyISAM");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."transazioni` ENGINE = MyISAM");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."transazioniweb` ENGINE = MyISAM");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."utenti` ENGINE = MyISAM");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."versioni` ENGINE = MyISAM");
for ($num1 = 0 ; $num1 < $num_anni ; $num1++) {
$anno_mostra = risul_query($anni,$num1,'idanni');
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."costi$anno_mostra` CONVERT TO CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."costiprenota$anno_mostra` CONVERT TO CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."ntariffe$anno_mostra` CONVERT TO CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."periodi$anno_mostra` CONVERT TO CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."prenota$anno_mostra` CONVERT TO CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."prenotacanc$anno_mostra` CONVERT TO CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."rclientiprenota$anno_mostra` CONVERT TO CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."regole$anno_mostra` CONVERT TO CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."soldi$anno_mostra` CONVERT TO CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."prenotacanc$anno_mostra` CHANGE `idappartamenti` `idappartamenti` varchar(100) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."prenotacanc$anno_mostra` CHANGE `assegnazioneapp` `assegnazioneapp` varchar(4) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."prenotacanc$anno_mostra` CHANGE `codice` `codice` varchar(10) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."prenotacanc$anno_mostra` CHANGE `origine` `origine` varchar(70) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."prenotacanc$anno_mostra` CHANGE `conferma` `conferma` varchar(4) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."prenotacanc$anno_mostra` CHANGE `hostinserimento` `hostinserimento` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."prenota$anno_mostra` CHANGE `idappartamenti` `idappartamenti` varchar(100) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."prenota$anno_mostra` CHANGE `assegnazioneapp` `assegnazioneapp` varchar(4) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."prenota$anno_mostra` CHANGE `codice` `codice` varchar(10) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."prenota$anno_mostra` CHANGE `origine` `origine` varchar(70) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."prenota$anno_mostra` CHANGE `conferma` `conferma` varchar(4) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."prenota$anno_mostra` CHANGE `hostinserimento` `hostinserimento` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."regole$anno_mostra` CHANGE `app_agenzia` `app_agenzia` varchar(100) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."rclientiprenota$anno_mostra` CHANGE `parentela` `parentela` varchar(70) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."rclientiprenota$anno_mostra` CHANGE `hostinserimento` `hostinserimento` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."ntariffe$anno_mostra` CHANGE `nomecostoagg` `nomecostoagg` varchar(40) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."ntariffe$anno_mostra` CHANGE `tipo_ca` `tipo_ca` varchar(2) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."ntariffe$anno_mostra` CHANGE `associasett_ca` `associasett_ca` varchar(1) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."ntariffe$anno_mostra` CHANGE `numsett_ca` `numsett_ca` varchar(20) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."ntariffe$anno_mostra` CHANGE `variazione_ca` `variazione_ca` varchar(20) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."ntariffe$anno_mostra` CHANGE `mostra_ca` `mostra_ca` varchar(10) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."ntariffe$anno_mostra` CHANGE `letto_ca` `letto_ca` varchar(1) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."ntariffe$anno_mostra` CHANGE `regoleassegna_ca` `regoleassegna_ca` varchar(30) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."costi$anno_mostra` CHANGE `nome_cassa` `nome_cassa` varchar(70) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."costi$anno_mostra` CHANGE `hostinserimento` `hostinserimento` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."costiprenota$anno_mostra` CHANGE `tipo` `tipo` varchar(2) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."costiprenota$anno_mostra` CHANGE `nome` `nome` varchar(40) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."costiprenota$anno_mostra` CHANGE `associasett` `associasett` varchar(1) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."costiprenota$anno_mostra` CHANGE `letto` `letto` varchar(1) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."costiprenota$anno_mostra` CHANGE `variazione` `variazione` varchar(10) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."costiprenota$anno_mostra` CHANGE `varnumsett` `varnumsett` varchar(20) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."costiprenota$anno_mostra` CHANGE `vartariffeassociate` `vartariffeassociate` varchar(10) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."costiprenota$anno_mostra` CHANGE `hostinserimento` `hostinserimento` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."prenotacanc$anno_mostra` CHANGE `app_assegnabili` `app_assegnabili` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."prenotacanc$anno_mostra` CHANGE `cat_persone` `cat_persone` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."prenotacanc$anno_mostra` CHANGE `idprenota_compagna` `idprenota_compagna` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."prenotacanc$anno_mostra` CHANGE `tariffa` `tariffa` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."prenotacanc$anno_mostra` CHANGE `tariffesettimanali` `tariffesettimanali` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."prenotacanc$anno_mostra` CHANGE `incompatibilita` `incompatibilita` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."prenotacanc$anno_mostra` CHANGE `valuta` `valuta` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."prenotacanc$anno_mostra` CHANGE `metodo_pagamento` `metodo_pagamento` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."prenotacanc$anno_mostra` CHANGE `commento` `commento` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."prenotacanc$anno_mostra` CHANGE `id_anni_prec` `id_anni_prec` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."soldi$anno_mostra` CHANGE `motivazione` `motivazione` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."soldi$anno_mostra` CHANGE `id_pagamento` `id_pagamento` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."soldi$anno_mostra` CHANGE `metodo_pagamento` `metodo_pagamento` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."soldi$anno_mostra` CHANGE `valuta` `valuta` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."prenota$anno_mostra` CHANGE `app_assegnabili` `app_assegnabili` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."prenota$anno_mostra` CHANGE `cat_persone` `cat_persone` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."prenota$anno_mostra` CHANGE `idprenota_compagna` `idprenota_compagna` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."prenota$anno_mostra` CHANGE `tariffa` `tariffa` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."prenota$anno_mostra` CHANGE `tariffesettimanali` `tariffesettimanali` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."prenota$anno_mostra` CHANGE `incompatibilita` `incompatibilita` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."prenota$anno_mostra` CHANGE `valuta` `valuta` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."prenota$anno_mostra` CHANGE `metodo_pagamento` `metodo_pagamento` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."prenota$anno_mostra` CHANGE `commento` `commento` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."prenota$anno_mostra` CHANGE `id_anni_prec` `id_anni_prec` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."regole$anno_mostra` CHANGE `tariffa_chiusa` `tariffa_chiusa` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."regole$anno_mostra` CHANGE `tariffa_per_app` `tariffa_per_app` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."regole$anno_mostra` CHANGE `tariffa_per_utente` `tariffa_per_utente` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."regole$anno_mostra` CHANGE `tariffa_per_persone` `tariffa_per_persone` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."regole$anno_mostra` CHANGE `motivazione` `motivazione` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."regole$anno_mostra` CHANGE `motivazione2` `motivazione2` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."regole$anno_mostra` CHANGE `motivazione3` `motivazione3` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."ntariffe$anno_mostra` CHANGE `moltiplica_ca` `moltiplica_ca` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."ntariffe$anno_mostra` CHANGE `periodipermessi_ca` `periodipermessi_ca` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."ntariffe$anno_mostra` CHANGE `beniinv_ca` `beniinv_ca` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."ntariffe$anno_mostra` CHANGE `appincompatibili_ca` `appincompatibili_ca` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."ntariffe$anno_mostra` CHANGE `categoria_ca` `categoria_ca` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
$num_tariffe_tab = esegui_query("select nomecostoagg from $PHPR_TAB_PRE"."ntariffe$anno_mostra where idntariffe = 1");
$num_tariffe_tab = risul_query($num_tariffe_tab,0,'nomecostoagg');
for ($num2 = 1 ; $num2 <= $num_tariffe_tab ; $num2++) {
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."ntariffe$anno_mostra` CHANGE `tariffa$num2` `tariffa$num2` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
} # fine for $num2
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."costi$anno_mostra` CHANGE `nome_costo` `nome_costo` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."costi$anno_mostra` CHANGE `tipo_costo` `tipo_costo` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."costi$anno_mostra` CHANGE `valuta` `valuta` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."costi$anno_mostra` CHANGE `persona_costo` `persona_costo` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."costi$anno_mostra` CHANGE `provenienza_costo` `provenienza_costo` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."costi$anno_mostra` CHANGE `metodo_pagamento` `metodo_pagamento` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."costiprenota$anno_mostra` CHANGE `settimane` `settimane` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."costiprenota$anno_mostra` CHANGE `moltiplica` `moltiplica` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."costiprenota$anno_mostra` CHANGE `categoria` `categoria` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."costiprenota$anno_mostra` CHANGE `cat_persone` `cat_persone` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."costiprenota$anno_mostra` CHANGE `varmoltiplica` `varmoltiplica` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."costiprenota$anno_mostra` CHANGE `varperiodipermessi` `varperiodipermessi` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."costiprenota$anno_mostra` CHANGE `varbeniinv` `varbeniinv` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."costiprenota$anno_mostra` CHANGE `varappincompatibili` `varappincompatibili` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."costiprenota$anno_mostra` CHANGE `vartariffeincomp` `vartariffeincomp` text CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NULL");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."costi$anno_mostra` ENGINE = MyISAM");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."costiprenota$anno_mostra` ENGINE = MyISAM");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."ntariffe$anno_mostra` ENGINE = MyISAM");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."periodi$anno_mostra` ENGINE = MyISAM");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."prenota$anno_mostra` ENGINE = MyISAM");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."prenotacanc$anno_mostra` ENGINE = MyISAM");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."rclientiprenota$anno_mostra` ENGINE = MyISAM");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."regole$anno_mostra` ENGINE = MyISAM");
esegui_query("ALTER TABLE `$PHPR_DB_NAME`.`$PHPR_TAB_PRE"."soldi$anno_mostra` ENGINE = MyISAM");
} # fine for $num1
if ($disp_err_orig) ini_set('display_errors',$disp_err_orig);
} # fine if ($PHPR_DB_TYPE == "mysqli")
} # fine if ($versione_corrente < "3.01")

if ($versione_corrente < "3.02") {
$aggiornato = "SI";
$tableclienti = $PHPR_TAB_PRE."clienti";
$clienti_di = esegui_query("select idclienti,doc_inviati from $tableclienti where doc_inviati is not NULL and doc_inviati != '' ");
$num_clienti_di = numlin_query($clienti_di);
$oggi = date("Y-m-d",(time() + (C_DIFF_ORE * 3600)));
for ($num1 = 0 ; $num1 < $num_clienti_di ; $num1++) {
$doc_inviati = risul_query($clienti_di,$num1,'doc_inviati');
if (substr($doc_inviati,0,3) == "#@?") {
$idclienti = risul_query($clienti_di,$num1,'idclienti');
$doc_inviati = "#@?".str_replace("#@?","#|?$oggi#@?",substr($doc_inviati,3));
esegui_query("update $tableclienti set doc_inviati = '".aggslashdb($doc_inviati)."' where idclienti = '$idclienti' ");
} # fine if (substr($doc_inviati,0,3) == "#@?")
} # fine for $num1
} # fine if ($versione_corrente < "3.02")

if ($versione_corrente < "3.03") {
$aggiornato = "SI";
} # fine if ($versione_corrente < "3.03")

if ($versione_corrente < "3.04") {
$aggiornato = "SI";
$colore_sfondo_quadro_disponibilita = "";
$tablesessioni = $PHPR_TAB_PRE."sessioni";
$tablesessioni_temp = $PHPR_TAB_PRE."sessio";
esegui_query("alter table $tablesessioni add column idcliente varchar(42) ");
esegui_query("alter table $tablesessioni rename to $tablesessioni_temp ");
...
esegui_query("insert into $tablesessioni select idsessioni,idcliente,idutente,indirizzo_ip,tipo_conn,user_agent,ultimo_accesso from $tablesessioni_temp ");
esegui_query("drop table $tablesessioni_temp ");
$tabletransazioni = $PHPR_TAB_PRE."transazioni";
$tabletransazioni_temp = $PHPR_TAB_PRE."transaz";
esegui_query("alter table $tabletransazioni add column idcliente varchar(42) ");
esegui_query("alter table $tabletransazioni rename to $tabletransazioni_temp ");
...
esegui_query("insert into $tabletransazioni select idtransazioni,idcliente,idsessione,tipo_transazione,anno,spostamenti,dati_transazione1,dati_transazione2,dati_transazione3,dati_transazione4,dati_transazione5,dati_transazione6,dati_transazione7,dati_transazione8,dati_transazione9,dati_transazione10,dati_transazione11,dati_transazione12,dati_transazione13,dati_transazione14,dati_transazione15,dati_transazione16,dati_transazione17,dati_transazione18,dati_transazione19,dati_transazione20,dati_transazione21,dati_transazione22,ultimo_accesso from $tabletransazioni_temp ");
esegui_query("drop table $tabletransazioni_temp ");
$tabletransazioniweb = $PHPR_TAB_PRE."transazioniweb";
$tabletransazioniweb_temp = $PHPR_TAB_PRE."transazw";
esegui_query("alter table $tabletransazioniweb add column idcliente varchar(42) ");
esegui_query("alter table $tabletransazioniweb rename to $tabletransazioniweb_temp ");
esegui_query("create table $tabletransazioniweb (idtransazioni varchar(42) primary key, idcliente varchar(42), idsessione varchar(42), tipo_transazione varchar(5), anno integer, spostamenti text, dati_transazione1 text, dati_transazione2 text, dati_transazione3 text, dati_transazione4 text, dati_transazione5 text, dati_transazione6 text, dati_transazione7 text, dati_transazione8 text, dati_transazione9 text, dati_transazione10 text, dati_transazione11 text, dati_transazione12 text, dati_transazione13 text, dati_transazione14 text, dati_transazione15 text, dati_transazione16 text, dati_transazione17 text, dati_transazione18 text, dati_transazione19 text, dati_transazione20 text, dati_transazione21 text, dati_transazione22 text, ultimo_accesso $DATETIME)");
esegui_query("insert into $tabletransazioniweb select idtransazioni,idcliente,idsessione,tipo_transazione,anno,spostamenti,dati_transazione1,dati_transazione2,dati_transazione3,dati_transazione4,dati_transazione5,dati_transazione6,dati_transazione7,dati_transazione8,dati_transazione9,dati_transazione10,dati_transazione11,dati_transazione12,dati_transazione13,dati_transazione14,dati_transazione15,dati_transazione16,dati_transazione17,dati_transazione18,dati_transazione19,dati_transazione20,dati_transazione21,dati_transazione22,ultimo_accesso from $tabletransazioniweb_temp ");
esegui_query("drop table $tabletransazioniweb_temp ");
$minuti_durata_sessione = esegui_query("select valpersonalizza_num from ".$PHPR_TAB_PRE."personalizza where idpersonalizza = 'minuti_durata_sessione' and idutente = '1'");
$minuti_durata_sessione = risul_query($minuti_durata_sessione,0,'valpersonalizza_num');
if (!defined('C_MIN_SESSIONE')) define('C_MIN_SESSIONE',$minuti_durata_sessione);
if (!defined('C_USA_COOKIES')) define('C_USA_COOKIES',0);
esegui_query("insert into ".$PHPR_TAB_PRE."personalizza (idpersonalizza,idutente,valpersonalizza_num) values ('usa_cookies','1','0')");
esegui_query("insert into ".$PHPR_TAB_PRE."personalizza (idpersonalizza,idutente,valpersonalizza) values ('modo_invio_email','1','locale')");
$id_appartamenti = esegui_query("select idappartamenti from ".$PHPR_TAB_PRE."appartamenti order by idappartamenti ");
...
flock($fileaperto,3);
fclose($fileaperto);
} # fine if ($versione_corrente < "3.04")
...
HOTELDRUID version ".C_PHPR_VERSIONE_TXT.", Copyright (C) 2001-2022 Marco M. F. De Santis<br>
...
include(C_DATI_PATH."/lingua.php");
$lingua_mex = $lingua[1];
$anno = risul_query($anni,($num_anni - 1),'idanni');
...
<input class=\"sbutton\" type=\"submit\" value=\"".mex("Vai al menù principale","aggiorna.php")."\">
</div></form></div>";
