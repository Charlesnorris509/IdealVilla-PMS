...
if ($nome_tab_prenota == "prenota") $nome_tab_prenota = $PHPR_TAB_PRE.$nome_tab_prenota;
if (!isset($profondita['iniziale'])) $profondita['iniziale'] = "";
$tableprenota = $nome_tab_prenota.$anno.$profondita['iniziale'];
$tableperiodi = $PHPR_TAB_PRE."periodi".$anno;
$tableappartamenti = $PHPR_TAB_PRE."appartamenti";

if (empty($limiti_var['idperiodocorrente'])) $limiti_var['idperiodocorrente'] = calcola_id_periodo_corrente($anno);
if (!isset($limiti_var['tutti_fissi'])) {
...
} # fine if (!isset($limiti_var['tutti_fissi']))
if (!isset($limiti_var['lim_prenota_temp'])) {
...
} # fine if (!isset($limiti_var['lim_prenota_temp']))
...
if (!isset($limiti_var['ini']) or (string) $limiti_var['ini'] == "") $limiti_var['ini'] = ($limiti_var['n_fine'] + 1);
if (!isset($limiti_var['fine']) or (string) $limiti_var['fine'] == "") $limiti_var['fine'] = ($limiti_var['n_ini'] - 1);
$lista_comp = "";
$query_comp = "";
...
if (empty($app_prenota_id[$lista_comp[$num1]])) {
$query_comp .= "idprenota = '".$lista_comp[$num1]."' or ";
} # fine if (empty($app_prenota_id[$lista_comp[$num1]]))
} # fine for $num1
...
if ($ins_prenota and (!isset($app_prenota_id[$idprenota]) or !$app_prenota_id[$idprenota])) {
...
if ($debug == "on") echo "Aggiunta prenota $idprenota dal ".$inizio_prenota_id[$idprenota]." al ".$fine_prenota_id[$idprenota]." in ".$app_prenota_id[$idprenota]." assegnabile in ".fixset($app_assegnabili_id[$idprenota])."<br>";
} # fine if (strstr($dati_app['lista'],",".$app_prenota.","))
} # fine if ($ins_prenota and (!isset($app_prenota_id[$idprenota]) or !$app_prenota_id[$idprenota]))
} # fine for $num1
} # fine for $num_rip
...
if (isset($prenota_in_app_sett[$app][$num1]) and $prenota_in_app_sett[$app][$num1]) {
$num_pp++;
$prenota_presente[$num_pp] = $prenota_in_app_sett[$app][$num1];
$num1 = $fine_prenota_id[$prenota_in_app_sett[$app][$num1]];
} # fine if (isset($prenota_in_app_sett[$app][$num1]) and $prenota_in_app_sett[$app][$num1])
} # fine for $num1
if (isset($prenota_presente)) return $prenota_presente;
else return null;
...
foreach ($app_orig_prenota_id as $idprenota => $app_prenota) if (!strcmp((string) $app_prenota_id[$idprenota],"")) $fatto = 0;
if ($fatto) {
reset($app_orig_prenota_id);
...
if (!empty($prenota_in_app_sett[$dati_app['posizione'][$num2]][$num1])) $num_prenota_presenti_in_settimana++;
} # fine for $num2
...
if (!empty($app_assegnabili_id[$prenotainperiodo[1]])) {
...
if (empty($app_controlla[$app_assegnabili_prenota[$num2]])) {
$app_controlla[$app_assegnabili_prenota[$num2]] = "SI";
$app_controlla_tmp[$app_assegnabili_prenota[$num2]] = "SI";
} # fine if (empty($app_controlla[$app_assegnabili_prenota[$num2]]))
} # fine for $num2
} # fine if (!empty($app_assegnabili_id[$prenotainperiodo[1]]))
...
for ($num1 = $idinizio ; $num1 <= $idfine ; $num1++) {
for ($num2 = 0 ; $num2 < $dati_app['totapp'] ; $num2++) {
if (isset($prenota_in_app_sett[$dati_app['posizione'][$num2]][$num1])) {
$pren_in_sett = $prenota_in_app_sett[$dati_app['posizione'][$num2]][$num1];
if ($pren_in_sett and !isset($pren_pres_in_lista[$pren_in_sett])) {
$pren_pres_in_lista[$pren_in_sett] = 1;
$lista_prenota_periodo[$num_lista_pren_per] = $pren_in_sett;
$num_lista_pren_per++;
} # fine if ($pren_in_sett and !isset($pren_pres_in_lista[$pren_in_sett]))
} # fine if (isset($prenota_in_app_sett[$dati_app['posizione'][$num2]][$num1]))
} # fine for $num2
} # fine for $num1
...
$app_richiesti[$id_ric] = fixset($app_assegnabili_id[$id_comp]);
if (empty($app_assegnabili_id[$id_comp])) $app_richiesti[$id_ric] = $app_prenota_id[$id_comp];
elseif ($app_assegnabili_id[$id_comp] == "v") $app_richiesti[$id_ric] = ",tutti,";
...
if (!isset($limiti_var_orig['ini']) or $limiti_var['ini'] < $limiti_var_orig['ini'] or $limiti_var['fine'] > $limiti_var_orig['fine']) {
if (@is_array($app_orig_prenota_id)) {
reset($app_orig_prenota_id);
foreach ($app_orig_prenota_id as $idp => $app) {
if (!strcmp(fixstr($app_prenota_id[$idp]),"")) {
$app_prenota_id[$idp] = $app;
for ($num1 = $inizio_prenota_id[$idp] ; $num1 <= $fine_prenota_id[$idp] ; $num1++) {
$prenota_in_app_sett[$app][$num1] = $idp;
} # fine for $num1
} # fine if (!strcmp(fixstr($app_prenota_id[$idp]),""))
} # fine foreach ($app_orig_prenota_id as $idp => $app)
} # fine if (@is_array($app_orig_prenota_id))
} # fine if (!isset($limiti_var_orig['ini']) or $limiti_var['ini'] < $limiti_var_orig['ini'] or...
...
if (empty($app_richiesti_vett['id'])) {
...
} # fine if (empty($app_richiesti_vett['id']))
else {
...
} # fine else if (empty($app_richiesti_vett['id']))
unset($app_richiesti);
...
} # fine else if (!empty($app_richiesti[',numero,']))
...
if (!isset($profondita['controllato_tutti_occupati']) or $profondita['controllato_tutti_occupati'] != "SI") {
...
} # fine if (!isset($profondita['controllato_tutti_occupati']) or $profondita[controllato_tutti_occupati] != "SI")
...
if (isset($app_richiesti_vett[',vicini,']) and $app_richiesti_vett[',vicini,'] == "SI") {
if (empty($app_assegnabili_id[0][0])) {
$app_assegnabili_id[0][0] = $app_richiesti_vett;
$ass00 = 1;
} # fine if (empty($app_assegnabili_id[0][0]))
if (empty($app_richiesti_vett[',succ_non_vicino,'][$app_richiesti_vett['id']])) {
$app_controllati = ",";
for ($num1 = 0 ; $num1 < $dati_app['totapp'] ; $num1++) {
$numapp = $dati_app['posizione'][$num1];
if (isset($dati_app['vicini'][$numapp]) and $dati_app['vicini'][$numapp] == $numapp and (empty($app_richiesti) or (isset($app_richiesti[$numapp]) and $app_richiesti[$numapp] == "SI"))) $app_non_liberabili .= "$numapp,";
} # fine for $num1
if ($app_richiesti_vett['id'] == 1 or !empty($app_richiesti_vett[',succ_non_vicino,'][($app_richiesti_vett['id'] - 1)])) {
$num_gruppo = 0;
$num_vicini_tot = 0;
...
if (!empty($app_richiesti_vett[',succ_non_vicino,'][$num1]) and $num1 != $app_richiesti_vett[',numero,']) {
...
} # fine if (!empty($app_richiesti_vett[',succ_non_vicino,'][$num1]) and $num1 != $app_richiesti_vett[',numero,'])
} # fine for $num1
if ($num_vicini_gruppo[$num_gruppo] >= $num_vicini_gruppo[0]) $num_vicini_tot += $num_vicini_gruppo[$num_gruppo];
...
for ($num1 = 0 ; $num1 < $dati_app['totapp'] ; $num1++) {
$numapp = $dati_app['posizione'][$num1];
if ((empty($app_richiesti) or (isset($app_richiesti[$numapp]) and $app_richiesti[$numapp] == "SI")) and !strstr($app_controllati,",$numapp,") and !strstr($app_non_liberabili,",$numapp,")) {
...
if (empty($app_assegnabili_id[$prenotainperiodo[$num4]])) {
$num_vicini_disp--;
break;
} # fine if (empty($app_assegnabili_id[$prenotainperiodo[$num4]]))
...
} # fine if ($app_richiesti_vett['id'] == 1 or !empty($app_richiesti_vett[',succ_non_vicino,'][($app_richiesti_vett['id'] - 1)]))
} # fine if (empty($app_richiesti_vett[',succ_non_vicino,'][$app_richiesti_vett['id']]))
} # fine if (isset($app_richiesti_vett[',vicini,']) and $app_richiesti_vett[',vicini,'] == "SI")
...
for ($num1 = 0 ; $num1 < $dati_app['totapp'] ; $num1++) {
$numapp = $dati_app['posizione'][$num1];
if (empty($app_richiesti) or (isset($app_richiesti[$numapp]) and $app_richiesti[$numapp] == "SI")) {
...
} # fine if (empty($app_richiesti) or (isset($app_richiesti[$numapp]) and $app_richiesti[$numapp] == "SI"))
} # fine for $num1
} # fine if ($fatto != "SI")
...
} # fine if (empty($limiti_var[s_ini]))
...
if (!empty($app_assegnabili_id[$idprenotainperiodo]) and $app_assegnabili_id[$idprenotainperiodo] != "v") {
...
} # fine if (!empty($app_assegnabili_id[$idprenotainperiodo]) and $app_assegnabili_id[$idprenotainperiodo] != "v")
if (!empty($app_assegnabili_id[$idprenotainperiodo])) {
...
} # fine if (!empty($app_assegnabili_id[$idprenotainperiodo]))
else $mobile = "NO";
} # fine for $num2
...
if ((isset($app_assegnabili_id[$idprenotainperiodo]) and $app_assegnabili_id[$idprenotainperiodo] == "v") or (isset($app_assegnabili[$idprenotainperiodo][$numapp2]) and $app_assegnabili[$idprenotainperiodo][$numapp2] == "SI")) {
...
if (empty($app_assegnabili_id[$idprenotainperiodo2])) $prenota_fissa_presente = "SI";
else {
...
} # fine if ((isset($app_assegnabili_id[$idprenotainperiodo]) and $app_assegnabili_id[$idprenotainperiodo] == "v") or...
} # fine if ($num3 != $num1)
} # fine for $num3

if (!isset($liberabile[$idprenotainperiodo]) or $liberabile[$idprenotainperiodo] != "SI") $tutti_liberabili = "NO";
if ((!isset($liberabile[$idprenotainperiodo]) or $liberabile[$idprenotainperiodo] != "SI") and (!isset($scambiabile[$idprenotainperiodo]) or $scambiabile[$idprenotainperiodo] != "SI")) $mobile2 = "NO";
} # fine for $num2
...
if ($fatto == "SI" and !empty($app_richiesti_vett[',numero,'])) {
...
if (isset($app_richiesti_vett[',vicini,']) and $app_richiesti_vett[',vicini,'] == "SI" and empty($app_richiesti_vett[',succ_non_vicino,'][$app_richiesti_vett['id']])) {
$fatto = "NO";
...
if (!isset($provato[$idinizio_vett[$num1]][$idfine_vett[$num1]][$app_richiesti_vett[$num1]])) {
$provato[$idinizio_vett[$num1]][$idfine_vett[$num1]][$app_richiesti_vett[$num1]] = 1;
$n_app_prenota_id2 = $n_app_prenota_id;
$n_prenota_in_app_sett2 = $n_prenota_in_app_sett;
$n_app_richiesti_vett = $app_richiesti_vett;
if (!isset($n_app_richiesti_vett[',app_vicini,']) or (string) $n_app_richiesti_vett[',app_vicini,'] == "") $n_app_richiesti_vett[',app_vicini,'] = $dati_app['vicini'][$appartamento];
else {
...
} # fine else if (!isset($n_app_richiesti_vett[',app_vicini,']) or (string) $app_richiesti_vett[',app_vicini,'] == "")
...
} # fine if (!isset($provato[$idinizio_vett[$num1]][$idfine_vett[$num1]][$app_richiesti_vett[$num1]]))
} # fine for $num1
if ($fatto != "SI") $riprova_senza_app_liberato = $appartamento;
} # fine if (isset($app_richiesti_vett[',vicini,']) and $app_richiesti_vett[',vicini,'] == "SI" and...
...
} # fine if ($fatto == "SI" and !empty($app_richiesti_vett[',numero,']))


# Se si era liberato un appartamento ma non se ne sono trovati altri vicini riprovo
if (!empty($riprova_senza_app_liberato) and ($limiti_var['t_limite'] + 20) >= time()) {
$n_lista_app_ric = "";
for ($num1 = 0 ; $num1 < $dati_app['totapp'] ; $num1++) {
$numapp = $dati_app['posizione'][$num1];
if ($numapp != $riprova_senza_app_liberato and (!isset($riprova_app[$numapp]) or $riprova_app[$numapp] != "NO")) $n_lista_app_ric .= $numapp.",";
} # fine for $num1
...
} # fine if (!empty($riprova_senza_app_liberato) and ($limiti_var['t_limite'] + 20) >= time())


if (isset($primo_ciclo) and $fatto == "SI") {
$risul_agg = aggiorna_tableprenota($app_prenota_id,$app_orig_prenota_id,$tableprenota);
if (!$risul_agg) $fatto = "NO";
} # fine if (isset($primo_ciclo) and $fatto == "SI")
if (!empty($ass00)) $app_assegnabili_id[0][0] = "";
