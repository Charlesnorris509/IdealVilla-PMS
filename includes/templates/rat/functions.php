...
function crea_modello_rat ($percorso_cartella_modello,$anno_modello,$PHPR_TAB_PRE,$pag,$lingua_modello,$silenzio,$fr_frase,$frase,$num_frasi,$tipo_periodi,$lingua_orig="") {
...
global $m_stile_soldi,$m_stile_data,$anteponi_nome_valuta,$mostra_caparra,$modello_esistente,$cambia_frasi,$template_data_dir,$template_file_name,$parola_settimane,$id_utente,$lingua_mex;
global $apertura_tag_font,$chiusura_tag_font,$stile_tabella_tariffe,$m_valuta_sing,$m_valuta_plur,$prima_parte_html,$ultima_parte_html,$file_css_frame,$file_js_frame,$tema_modello,$id_transazione;
$tablenometariffe_modello = $PHPR_TAB_PRE."ntariffe".$anno_modello;
...
if ($id_utente != 1) {
global $anno,$attiva_tariffe_consentite,$tariffe_consentite_vett,$attiva_costi_agg_consentiti,$costi_agg_consentiti_vett;
...
if ($tipo_transazione == "cpweb" and risul_query($dati_transazione,0,'spostamenti') == "rat") {
...
} # fine if ($tipo_transazione == "cpweb" and risul_query($dati_transazione,0,'spostamenti') == "rat")
...
$costi_attivati .= " ".$dati_ca[$numca]['id']." => \"$SI\",";
$nome_costo_imposto = "nome_costo_imposto".$dati_ca[$numca]['id'];
...
if (strtoupper(\$".mex("var_mostra_date_passate",$pag).") == \"$NO\") \$mostra_date_passate = \"NO\";
else \$mostra_date_passate = \"SI\";
...
if (numlin_query($inizioperiodo)) $inizioperiodo = risul_query($inizioperiodo,0,'datainizio');
else $inizioperiodo = null;
$fineperiodo = esegui_query("select datafine from $tableperiodi where idperiodi = '$fineperiodo' ");
if (numlin_query($fineperiodo)) $fineperiodo = risul_query($fineperiodo,0,'datafine');
else $fineperiodo = null;
