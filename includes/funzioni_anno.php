...
global $lingua_mex,$LIKE,$ILIKE,$PHPR_DB_TYPE,$PHPR_DB_NAME,$HOSTNAME;
...
esegui_query("alter database $PHPR_DB_NAME default character set '$character_set_db' collate '$collation_db'");
...
$tableprenotazionicanc = $PHPR_TAB_PRE."prenotacanc".$anno;
esegui_query("create table $tableprenotazionicanc ($col_tableprenota)");
$col_tableprenota = estrai_col_tabella($col_tableprenota);

# Creo la tabella con i costi aggiuntivi delle prenotazioni
$tablecostiprenota = $PHPR_TAB_PRE."costiprenota".$anno;
$col_tablecostiprenota = "idcostiprenota integer primary key, idprenota integer, tipo varchar(2), nome varchar(40), valore float8, valore_perc float8, arrotonda float4, tasseperc float4, associasett varchar(1), settimane text, moltiplica text, categoria text, letto varchar(1), cat_persone text, numlimite integer, idntariffe integer, variazione varchar(10), varmoltiplica text, varnumsett varchar(20), varperiodipermessi text, varbeniinv text, varappincompatibili text, vartariffeassociate varchar(10), vartariffeincomp text, datainserimento $DATETIME, hostinserimento varchar(50), utente_inserimento integer";
...
$col_tablecosti = "idcosti integer unique,nome_costo text, val_costo float8, tipo_costo text, valuta text, costo_valuta float8, nome_cassa varchar(70), persona_costo text, provenienza_costo text, metodo_pagamento text, datainserimento $DATETIME, hostinserimento varchar(50), utente_inserimento integer";
...
$col_tablesoldi = "idsoldi integer unique, motivazione text, id_pagamento text, metodo_pagamento text, saldo_prenota float8, saldo_cassa float8, soldi_prima float8, valuta text, saldo_valuta float8, data_inserimento $DATETIME, utente_inserimento integer";
...
$tableprenotacanc_prec = $PHPR_TAB_PRE."prenotacanc".$anno_prec;
...
$tabelle_lock = array($tableanni,$tableprenota_prec,$tableprenotazioni,$tableprenotacanc_prec,$tableprenotazionicanc,$tablecostiprenota_prec,$tablecostiprenota,$tablerclientiprenota_prec,$tablerclientiprenota,$tablenometariffe,$tableperiodi,$tablerelclienti,$tablecosti_prec,$tablecosti,$tableregole,$tablesoldi_prec,$tablesoldi,$tableinterconnessioni,$tablemessaggi,$tablepersonalizza,$tableprivilegi,$tabletransazioni);
...
$idprenota_corr = 0;
$nuovo_idprenota_iniziata = array();
...
$max_idprenotacanc_prec = esegui_query("select max(idprenota) from $tableprenotacanc_prec");
$max_idprenotacanc_prec = risul_query($max_idprenotacanc_prec,0,0) + 1;
if ($max_idprenotacanc_prec > $max_idprenota_prec) $max_idprenota_prec = $max_idprenotacanc_prec;
...
$pagato = risul_query($prenota_gia_iniziate,$num1,'pagato');
$tariffa_tot = risul_query($prenota_gia_iniziate,$num1,'tariffa_tot');
$id_anni_prec = risul_query($prenota_gia_iniziate,$num1,'id_anni_prec');
...
if ($pagato) esegui_query("update $tableprenota set pagato = '".aggslashdb($pagato)."' where idprenota = '$idprenota_corr'");
if ($tariffa_tot) esegui_query("update $tableprenota set tariffa_tot = '".aggslashdb($tariffa_tot)."' where idprenota = '$idprenota_corr'");
esegui_query("update $tableprenota set id_anni_prec = '".aggslashdb($id_anni_prec)."' where idprenota = '$idprenota_corr'");
...
$nuovo_idprenota_iniziata[$idprenota] = $idprenota_corr;
} # fine for $num1

$nuovo_idprenota = array();
$nuovo_iddatainizio = array();
$nuovo_iddatafine = array();
...
$prenotacanc_importate = esegui_query("select * from $tableprenotacanc_prec where iddatainizio >= '$id_data_ini_periodi_prec' and iddatafine <= '$id_data_fine_periodi_prec' order by idprenota ");
$num_prenotacanc_importate = numlin_query($prenotacanc_importate);
for ($num1 = 0 ; $num1 < $num_prenotacanc_importate ; $num1 = $num1 + 1) {
$idprenota = risul_query($prenotacanc_importate,$num1,'idprenota');
$iddatainizio = risul_query($prenotacanc_importate,$num1,'iddatainizio');
$iddatainizio = $iddatainizio - $id_data_ini_periodi_prec + 1;
$iddatafine = risul_query($prenotacanc_importate,$num1,'iddatafine');
$iddatafine = $iddatafine - $id_data_ini_periodi_prec + 1;
$idclienti = risul_query($prenotacanc_importate,$num1,'idclienti');
$lista_clienti_importati .= "and idclienti != '$idclienti' ";
$id_anni_prec = risul_query($prenotacanc_importate,$num1,'id_anni_prec');
if (!$id_anni_prec) $id_anni_prec = ";";
$id_anni_prec .= "$anno_prec,$idprenota;";
$idprenota_corr++;
esegui_query("update $tableprenotacanc_prec set idprenota = '$max_idprenota_prec' where idprenota = '$idprenota'");
esegui_query("insert into $tableprenotacanc select $col_tableprenota from $tableprenotacanc_prec where idprenota = '$max_idprenota_prec' ");
esegui_query("update $tableprenotacanc set iddatainizio = '$iddatainizio' where idprenota = '$max_idprenota_prec'");
esegui_query("update $tableprenotacanc set iddatafine = '$iddatafine' where idprenota = '$max_idprenota_prec'");
esegui_query("update $tableprenotacanc set id_anni_prec = '$id_anni_prec' where idprenota = '$max_idprenota_prec'");
esegui_query("update $tableprenotacanc set idprenota = '$idprenota_corr' where idprenota = '$max_idprenota_prec'");
...
for ($num1 = 0 ; $num1 < $num_id_costi_agg_importati ; $num1++) {
...
$settimane = (string) risul_query($costi_agg_importati,$num1,'settimane');
...
if (strcmp((string) $tariffa,"")) esegui_query("update $tableperiodi set tariffa$num2 = '$tariffa' where idperiodi = $idperiodi ");
$tariffap = (string) risul_query($tariffe_importate,$num1,"tariffa".$num2."p");
if (strcmp((string) $tariffap,"")) esegui_query("update $tableperiodi set tariffa$num2"."p = '$tariffap' where idperiodi = $idperiodi ");
} # fine for $num2
} # fine for $num1
...
if (strcmp((string) $motivazione,"")) esegui_query("update $tableregole set motivazione = '".aggslashdb($motivazione)."' where idregole = '$idregole' ");
if (strcmp((string) $motivazione2,"")) esegui_query("update $tableregole set motivazione2 = '".aggslashdb($motivazione2)."' where idregole = '$idregole' ");
if (strcmp((string) $motivazione3,"")) esegui_query("update $tableregole set motivazione3 = '".aggslashdb($motivazione3)."' where idregole = '$idregole' ");
} # fine for $num1
...
if (!empty($nuovo_idprenota[$motiv[3]])) {
$idsoldi_corr++;
...
} # fine if (!empty($nuovo_idprenota[$motiv[3]]))
} # fine for $num1
...
if (!empty($nuovo_idprenota[$lista_pren[$num2]])) $n_lista_pren .= $nuovo_idprenota[$lista_pren[$num2]].",";
else $agg_mess = 0;
} # fine for $num2
if ($agg_mess) {
...
if (@is_dir($dir_salva)) {
$num_contr = risul_query($dirs_salva,$num1,'numero');
...
$compress = esegui_query("select testo from ".$PHPR_TAB_PRE."contratti where numero = '$num_contr' and tipo = 'compress' ");
if (numlin_query($compress)) $compress = risul_query($compress,0,'testo');
else $compress = "";
$suff_file = "html";
if ($tipo_contratto == "contrrtf") $suff_file = "rtf";
if ($tipo_contratto == "contrtxt") $suff_file = "txt";
if ($compress) $suff_file .= ".gz";*/
$nomefile = esegui_query("select testo from ".$PHPR_TAB_PRE."contratti where numero = '$num_contr' and tipo = 'nomefile' ");
if (numlin_query($nomefile)) $nomefile = risul_query($nomefile,0,'testo');
else $nomefile = "";
...
if ($nomefile) $nome_contratto = $nomefile;
else $nome_contratto = $nomi_contr_ut[$idutente_contr][$num_contr];
if (empty($prefissi_contr[$dir_salva."/".$nome_contratto])) {
...
$contr_no_compr = substr($contr_corr,0,-3);
$suff_compr = ".gz";
} # fine if (substr($contr_corr,-3) == ".gz")
else $contr_no_compr = $contr_corr;
if (substr($contr_no_compr,-4) == ".rtf") $suff_file = "rtf".$suff_compr;
if (substr($contr_no_compr,-5) == ".html") $suff_file = "html".$suff_compr;
if (substr($contr_no_compr,-4) == ".txt") $suff_file = "txt".$suff_compr;
if ($suff_file) {
$resto_nome_contr = substr($contr_corr,strlen($nome_contratto));
if (substr($resto_nome_contr,0,6) == "_$anno"."_") {
if (preg_replace("/_[0-9]{4,4}_[0-9]{5,8}(-[0-9]{5,8})?(_[0-9]+(-[0-9]+|\+[0-9]+(\+[0-9]+)?)?)*\.$suff_file/","",$resto_nome_contr) == "") {
$num_prog_contr = explode("_",$resto_nome_contr);
$num_prog_contr = $num_prog_contr[2];
if (substr($num_prog_contr,(-1 * (strlen($suff_file) + 1))) == ".".$suff_file) $num_prog_contr = substr($num_prog_contr,0,(-1 * (strlen($suff_file) + 1)));
$num_pren_esist = substr($resto_nome_contr,(7 + strlen($num_prog_contr)));
...
if ($file_dat_ext and is_file($dir_salva."/".substr($contr_corr,0,(-1 * (strlen($suff_file) + 1))).".dat")) {
$nome_file_dat = substr($contr_corr,0,(-1 * (strlen($suff_file) + 1))).".dat";
$file_dat = file("$dir_salva/$nome_file_dat");
for ($num1 = 0 ; $num1 < count($file_dat) ; $num1++) {
if (substr($file_dat[$num1],0,3) == "rn:") {
if (!$num_pren_esist) $num_pren_esist = trim(substr($file_dat[$num1],3));
$file_dat[$num1] = "";
} # fine if (substr($file_dat[$num1],0,3) == "rn:")
} # fine for $num1
} # fine if ($file_dat_ext and is_file($dir_salva."/".substr($contr_corr,0,(-1 * (strlen($suff_file) + 1))).".dat"))
else {
$nome_file_dat = "";
$file_dat = array();
} # fine else if ($file_dat_ext and is_file($dir_salva."/".substr($contr_corr,0,(-1 * (strlen($suff_file) + 1))).".dat"))
if ($num_pren_esist) {
$n_lista_pren = "";
$num_pren_esist = explode("_",$num_pren_esist);
for ($num3 = 0 ; $num3 < count($num_pren_esist) ; $num3++) {
if (strstr($num_pren_esist[$num3],"+")) $n_lista_pren.= "_".$num_pren_esist[$num3];
else {
...
if (empty($nuovo_idprenota[$num4])) {
if (($num4 - 1) > $num_pren_esist2[0]) $n_lista_pren .= "-".$nuovo_idprenota[($num4 - 1)];
$n_lista_pren .= "_$anno_prec+$num4";
if (!empty($nuovo_idprenota_iniziata[$num4])) $n_lista_pren .= "+".$nuovo_idprenota_iniziata[$num4];
$num_pren_esist2[0] = ($num4 + 1);
} # fine if (empty($nuovo_idprenota[$num4]))
else {
if ($num4 == $num_pren_esist2[0]) $n_lista_pren .= "_".$nuovo_idprenota[$num4];
elseif ($num4 == $fine_for) $n_lista_pren .= "-".$nuovo_idprenota[$num4];
} # fine else if (empty($nuovo_idprenota[$num4]))
} # fine for $num4
} # fine else if (strstr($num_pren_esist[$num3],"+"))
} # fine for $num3

if (strlen($nome_contratto.substr($resto_nome_contr,0,(6 + strlen($num_prog_contr))).$n_lista_pren.".$suff_file") < 235) {
if (!empty($file_dat)) $n_nome_contr = $nome_contratto.substr($resto_nome_contr,0,(6 + strlen($num_prog_contr)))."_1+1".$n_lista_pren;
else $n_nome_contr = $nome_contratto.substr($resto_nome_contr,0,(6 + strlen($num_prog_contr))).$n_lista_pren;
} # fine if (strlen($nome_contratto.substr($resto_nome_contr,0,(6 + strlen($num_prog_contr))).$n_lista_pren.".$suff_file") < 235)
else {
$n_nome_contr = $nome_contratto.substr($resto_nome_contr,0,(6 + strlen($num_prog_contr)))."_1+1";
$file_dat[count($file_dat)] = "rn:".substr($n_lista_pren,1)."\n";
} # fine if (strlen($nome_contratto.substr($resto_nome_contr,0,(6 + strlen($num_prog_contr))).$n_lista_pren.".$suff_file") < 235)
rename($dir_salva."/".$contr_corr,$dir_salva."/".$n_nome_contr.".$suff_file");
if ($nome_file_dat) unlink($dir_salva."/".$nome_file_dat);
if (!empty($file_dat)) {
$fdat = fopen($dir_salva."/".$n_nome_contr.".dat","w+");
flock($fdat,2);
fwrite($fdat,implode($file_dat));
flock($fdat,3);
fclose($fdat);
} # fine if (!empty($file_dat))
} # fine if ($num_pren_esist)
} # fine if (preg_replace("/_[0-9]{4,4}_[0-9]{5,8}(-[0-9]{5,8})?(_[0-9]+(-[0-9]+|\+[0-9]+(\+[0-9]+)?)?)*\.$suff_file/","",$resto_nome_contr) == "")
...
closedir($contr_dir);
} # fine if (empty($prefissi_contr[$dir_salva."/".$nome_contratto]))
} # fine for $num2
...
if ($tipo_costo == "e") $cassa_prec[$utente_costo][$nome_cassa] = fixset($cassa_prec[$utente_costo][$nome_cassa]) + (double) $val_costo;
else $cassa_prec[$utente_costo][$nome_cassa] = fixset($cassa_prec[$utente_costo][$nome_cassa]) - (double) $val_costo;
} # fine for $num1
...
for ($num1 = 1 ; $num1 <= $num_tariffe_tab ; $num1++) {
$prezzi_imp_int = (string) risul_query($prezzi_tar_importati,0,"tariffa$num1");
...
if ($character_set_db_orig and ($character_set_db != $character_set_db_orig or $collation_db != $collation_db_orig)) @esegui_query("alter database $PHPR_DB_NAME default character set '$character_set_db_orig' collate '$collation_db_orig'");
