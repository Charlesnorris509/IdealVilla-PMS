...
# head
$body_bgcolor = "#dddddd";
global $vers_hinc,$mobile_device;
if (!isset($show_bar)) $show_bar = "";
if (!isset($tema_corr)) $tema_corr = $tema[$id_utente];
if (isset($senza_colori) or ($pag == "visualizza_contratto.php" and $show_bar == "NO")) $body_bgcolor = "#ffffff";
...
<html lang=\"$lingua_mex\">
<head>
<meta http-equiv=\"Content-Type\" content=\"text/html; charset=utf-8\" >
<meta name=\"viewport\" content=\"initial-scale=$iscale\">
<title> $titolo </title>";
if ($pag == "visualizza_contratto.php" and !empty($extra_head)) echo $extra_head;
...
:root { --t1color: $t1color; }
.t1color { background-color: $t1color; }
";
if (empty($cookie_sess)) echo "a:link { user-select: none; -moz-user-select: none; -webkit-user-select: none; -ms-user-select: none; }
";
echo "</style>
<link rel=\"stylesheet\" type=\"text/css\" href=\"./base.css$vers_hinc\" media=\"all\">
...
";
if (isset($base_js)) echo "<script type=\"text/javascript\" src=\"./base.js$vers_hinc\">
</script>
";
$hstyle = "";
if ($show_bar == "NO") $hstyle .=  "html { overflow-y: auto; }
#contentbox { overflow: visible; width: 99%; height: 98%; margin-right: auto; margin-left: auto; padding-top: 4px; }
";
if (isset($start_time)) $hstyle .=  "#contentbox { height: 95%; }
";
if (mex("insert-ddw","head.php") != "100px") $hstyle .=  ".nb_ins ul ul a, .nb_ins ul ul a:visited, .nb_ins ul ul a:hover { min-width: ".mex("insert-ddw","head.php")."; }
";
if (mex("tables-ddw","head.php") != "100px") $hstyle .=  ".nb_tab ul ul a, .nb_tab ul ul a:visited, .nb_tab ul ul a:hover { min-width: ".mex("tables-ddw","head.php")."; }
";
if (mex("months-ddw","head.php") != "86px") $hstyle .=  ".nb_m0 ul ul a, .nb_m0 ul ul a:visited, .nb_m0 ul ul a:hover { min-width: ".mex("months-ddw","head.php")."; }
";
if (mex("reservations-ddw","head.php") != "150px") $hstyle .=  ".nb_res ul ul a, .nb_res ul ul a:visited, .nb_res ul ul a:hover, .nb_res ul ul div { min-width: ".mex("reservations-ddw","head.php")."; }
";
if (mex("clients-ddw","head.php") != "15em") $hstyle .=  ".fdrop { min-width: ".mex("clients-ddw","head.php")."; }
";
if (mex("rates-ddw","head.php") != "150px") $hstyle .=  ".nb_rat ul ul a, .nb_rat ul ul a:visited, .nb_rat ul ul a:hover { min-width: ".mex("rates-ddw","head.php")."; }
";
if (mex("configure-ddw","head.php") != "86px") $hstyle .=  ".nb_con ul ul a, .nb_con ul ul a:visited, .nb_con ul ul a:hover { min-width: ".mex("configure-ddw","head.php")."; }
";
if ($hstyle) echo "<style type=\"text/css\">
$hstyle</style>
";
...
echo "</head>
<body";
if (isset($drag_drop)) echo " ondragover=\"event.preventDefault();\" ondragenter=\"event.preventDefault();\" ondrop=\"event.preventDefault();drp_out();\"";
...
if (!isset($privilegi_globali_utente)) $privilegi_globali_utente = esegui_query("select * from $tableprivilegi where idutente = '$id_utente' and anno = '1'");
if (!isset($priv_mod_pers)) $priv_mod_pers = risul_query($privilegi_globali_utente,0,'priv_mod_pers');
if (!fixset($modifica_pers) and substr($priv_mod_pers,0,1) != "s") $modifica_pers = "NO";
if (!isset($priv_crea_backup)) $priv_crea_backup = substr($priv_mod_pers,1,1);
if (!isset($priv_crea_interconnessioni)) $priv_crea_interconnessioni = substr($priv_mod_pers,3,1);
if (!isset($priv_crea_pagineweb)) $priv_crea_pagineweb = substr($priv_mod_pers,8,1);
if (!isset($priv_gest_pass_cc)) $priv_gest_pass_cc = substr($priv_mod_pers,5,1);
if (!isset($priv_ins_clienti)) $priv_ins_clienti = risul_query($privilegi_globali_utente,0,'priv_ins_clienti');
if (!fixset($inserimento_nuovi_clienti) and substr($priv_ins_clienti,0,1) != "s") $inserimento_nuovi_clienti = "NO";
if (!fixset($modifica_clienti) and substr($priv_ins_clienti,1,1) != "s" and substr($priv_ins_clienti,1,1) != "p") $modifica_clienti = "NO";
if (!fixset($vedi_clienti) and substr($priv_ins_clienti,2,1) != "s" and substr($priv_ins_clienti,2,1) != "p") $vedi_clienti = "NO";
if (!isset($priv_vedi_messaggi)) {
$priv_messaggi = risul_query($privilegi_globali_utente,0,'priv_messaggi');
$priv_vedi_messaggi = substr($priv_messaggi,0,1);
} # fine if (!isset($priv_vedi_messaggi))
if (!isset($priv_inventario)) $priv_inventario = risul_query($privilegi_globali_utente,0,'priv_inventario');
if (!isset($priv_vedi_beni_inv)) $priv_vedi_beni_inv = substr($priv_inventario,0,1);
if (!isset($priv_vedi_inv_mag)) $priv_vedi_inv_mag = substr($priv_inventario,2,1);
if (!isset($priv_vedi_inv_app)) $priv_vedi_inv_app = substr($priv_inventario,6,1);
if (!isset($priv_ins_prenota)) $priv_ins_prenota = risul_query($privilegi_annuali_utente,0,'priv_ins_prenota');
if (!isset($priv_ins_nuove_prenota)) $priv_ins_nuove_prenota = substr($priv_ins_prenota,0,1);
if (!isset($priv_ins_costi)) $priv_ins_costi = risul_query($privilegi_annuali_utente,0,'priv_ins_costi');
if (!isset($priv_ins_spese)) $priv_ins_spese = substr($priv_ins_costi,0,1);
if (!isset($priv_ins_entrate)) $priv_ins_entrate = substr($priv_ins_costi,1,1);
if (!isset($priv_mod_prenota)) $priv_mod_prenota = risul_query($privilegi_annuali_utente,0,'priv_mod_prenota');
if (!isset($priv_mod_prenotazioni)) $priv_mod_prenotazioni = substr($priv_mod_prenota,0,1);
if (!isset($priv_mod_costi_agg)) $priv_mod_costi_agg = substr($priv_mod_prenota,8,1);
if (!isset($priv_vedi_tab)) $priv_vedi_tab = risul_query($privilegi_annuali_utente,0,'priv_vedi_tab');
if (!isset($priv_vedi_tab_mesi)) $priv_vedi_tab_mesi = substr($priv_vedi_tab,0,1);
if (!isset($priv_vedi_tab_prenotazioni)) $priv_vedi_tab_prenotazioni = substr($priv_vedi_tab,1,1);
if (!isset($priv_vedi_tab_costi)) $priv_vedi_tab_costi = substr($priv_vedi_tab,2,1);
if (!isset($priv_vedi_tab_periodi)) $priv_vedi_tab_periodi = substr($priv_vedi_tab,3,1);
if (!isset($priv_vedi_tab_regole)) $priv_vedi_tab_regole = substr($priv_vedi_tab,4,1);
if (!isset($priv_vedi_tab_appartamenti)) $priv_vedi_tab_appartamenti = substr($priv_vedi_tab,5,1);
if (!isset($priv_vedi_tab_stat)) $priv_vedi_tab_stat = substr($priv_vedi_tab,6,1);
if (!isset($priv_vedi_tab_doc)) $priv_vedi_tab_doc = substr($priv_vedi_tab,7,1);
if (!isset($priv_ins_tariffe)) $priv_ins_tariffe = risul_query($privilegi_annuali_utente,0,'priv_ins_tariffe');
if (!isset($priv_mod_tariffe)) $priv_mod_tariffe = substr($priv_ins_tariffe,0,1);
if (!isset($priv_mod_reg1)) $priv_mod_reg1 = substr($priv_ins_tariffe,4,1);
if (!isset($priv_mod_reg2)) $priv_mod_reg2 = substr($priv_ins_tariffe,5,1);
if (!isset($priv_ins_costi_agg)) $priv_ins_costi_agg = substr($priv_ins_tariffe,1,1);
} # fine if ($anno_utente_attivato == "SI")
} # fine if ($id_utente != 1)
else {
if (!isset($anno_utente_attivato)) $anno_utente_attivato = "SI";
if (!isset($modifica_pers)) $modifica_pers = "SI";
if (!isset($priv_crea_backup)) $priv_crea_backup = "s";
if (!isset($priv_crea_interconnessioni)) $priv_crea_interconnessioni = "s";
if (!isset($priv_crea_pagineweb)) $priv_crea_pagineweb = "s";
if (!isset($priv_gest_pass_cc)) $priv_gest_pass_cc = "s";
if (!isset($inserimento_nuovi_clienti)) $inserimento_nuovi_clienti = "SI";
if (!isset($modifica_clienti)) $modifica_clienti = "SI";
if (!isset($vedi_clienti)) $vedi_clienti = "SI";
if (!isset($priv_vedi_messaggi)) $priv_vedi_messaggi = "s";
if (!isset($priv_vedi_beni_inv)) $priv_vedi_beni_inv = "s";
if (!isset($priv_vedi_inv_mag)) $priv_vedi_inv_mag = "s";
if (!isset($priv_vedi_inv_app)) $priv_vedi_inv_app = "s";
if (!isset($priv_ins_nuove_prenota)) $priv_ins_nuove_prenota = "s";
if (!isset($priv_ins_spese)) $priv_ins_spese = "s";
if (!isset($priv_ins_entrate)) $priv_ins_entrate = "s";
if (!isset($priv_mod_prenotazioni)) $priv_mod_prenotazioni = "s";
if (!isset($priv_mod_costi_agg)) $priv_mod_costi_agg = "s";
if (!isset($priv_vedi_tab_mesi)) $priv_vedi_tab_mesi = "s";
if (!isset($priv_vedi_tab_prenotazioni)) $priv_vedi_tab_prenotazioni = "s";
if (!isset($priv_vedi_tab_costi)) $priv_vedi_tab_costi = "s";
if (!isset($priv_vedi_tab_periodi)) $priv_vedi_tab_periodi = "s";
if (!isset($priv_vedi_tab_regole)) $priv_vedi_tab_regole = "s";
if (!isset($priv_vedi_tab_appartamenti)) $priv_vedi_tab_appartamenti = "s";
if (!isset($priv_vedi_tab_stat)) $priv_vedi_tab_stat = "s";
if (!isset($priv_vedi_tab_doc)) $priv_vedi_tab_doc = "s";
if (!isset($priv_mod_tariffe)) $priv_mod_tariffe = "s";
if (!isset($priv_mod_reg1)) $priv_mod_reg1 = "s";
if (!isset($priv_mod_reg2)) $priv_mod_reg2 = "s";
if (!isset($priv_ins_costi_agg)) $priv_ins_costi_agg = "s";
...
if (($pag == "tabella.php" or $pag == "tabella2.php" or $pag == "tabella3.php" or $pag == "visualizza_tabelle.php" or $pag == "storia_soldi.php") and !$mobile_device) $mostra_X = 1;


echo "<table class=\"nav_bar t1color\" width=\"100%\" cellpadding=\"0\" cellspacing=\"0\">
<tr>";

if (isset($mostra_X)) echo "<td style=\"width: 10px;\"></td>";
if (!empty($idprenota_origine)) $anno = $anno + 1;
if ($id_sessione) {
...
if ($priv_ins_spese == "s" or $priv_ins_entrate == "s" or $priv_mod_tariffe != "n" or $priv_ins_costi_agg != "n" or ($id_utente == 1 and !isset($installazione_subordinata)) or ($priv_mod_prenotazioni != "n" and $priv_mod_costi_agg == "s") or $inserimento_nuovi_clienti != "NO" or ($modifica_clienti != "NO" and $vedi_clienti != "NO") or ($mobile_device and $priv_ins_nuove_prenota == "s")) {
echo "<ul>";
...
if ($priv_mod_reg1 != "n" or $priv_mod_reg2 != "n") echo "<li><a id=\"nb_irul\" href=\"./crearegole.php?$sessione_anno_var\">&nbsp;<b>".mex("REGOLE","head.php")."</b>&nbsp;</a></li>";
...
if (($modifica_pers != "NO" or ($priv_crea_backup == "s" and !isset($installazione_subordinata)) or $priv_crea_pagineweb == "s" or ($priv_crea_interconnessioni == "s" and (!defined('C_CREA_SUBORDINAZIONI') or C_CREA_SUBORDINAZIONI != "NO")) or $priv_gest_pass_cc == "s") and !$mobile_device) {
...
if ($modifica_pers != "NO" or ($priv_crea_backup == "s" and !isset($installazione_subordinata)) or $priv_crea_pagineweb == "s" or ($priv_crea_interconnessioni == "s" and (!defined('C_CREA_SUBORDINAZIONI') or C_CREA_SUBORDINAZIONI != "NO"))) {
echo "<ul>";
if ($id_utente == 1 and !isset($installazione_subordinata)) echo "<li><a id=\"nb_use\" href=\"./gestione_utenti.php?$sessione_anno_var\">&nbsp;<b>".mex("UTENTI","head.php")."</b>&nbsp;</a></li>";
if ($priv_crea_pagineweb == "s") echo "<li><a id=\"nb_web\" href=\"./crea_modelli.php?$sessione_anno_var\">&nbsp;<b>".mex("SITO WEB","head.php")."</b>&nbsp;</a></li>";
...
if ($priv_crea_backup == "s" and !isset($installazione_subordinata)) echo "<li><a id=\"nb_bac\" href=\"./crea_backup.php?$sessione_anno_var\">&nbsp;<b>".mex("BACKUP","head.php")."</b>&nbsp;</a></li>";
...
echo "</ul>";
} # fine if ($modifica_pers != "NO" or ($priv_crea_backup == "s" and !isset($installazione_subordinata)) or
echo "</li></ul></div></td><td>";
} # fine if (($modifica_pers != "NO" or ($priv_crea_backup == "s" and !isset($installazione_subordinata)) or...
echo "</td></tr></table></td>";
...
$lista_var_X .= "&amp;".urlencode(key($_POST))."=".urlencode($_POST[key($_POST)]);
next($_POST);
} # fine for $num1
if (@is_array($_GET)) reset($_GET);
for($num1 = 0 ; $num1 < count($_GET); $num1++) {
$lista_var_X .= "&amp;".urlencode(key($_GET))."=".urlencode($_GET[key($_GET)]);
next($_GET);
} # fine for $num1
if ($lista_var_X) {
...
else echo "</div><table id=\"contentbox\" cellspacing=0 cellpadding=0><tr><td id=\"td_contentbox\" valign=\"top\">";
