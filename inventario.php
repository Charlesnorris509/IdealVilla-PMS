...
$var_pag[2] = 'idmag';
$var_pag[3] = 'url_enc';
$var_pag[4] = 'origine_vecchia';
$var_pag[5] = 'aggiorna_inventario';
$var_pag[6] = 'manda_form_tab';
$var_pag[7] = 'tipo_nuova_quantita';
$var_pag[8] = 'modifica_quantita';
$n_var_pag = 9;
...
$titolo = "HotelDruid: ".mex("Inventario",$pag);
...
if (empty($url_enc)) $idmag = htmlspecialchars($idmag);
...
if (!empty($aggiorna_inventario) and $priv_vedi_beni_inv != "n") {
...
if (!empty($manda_form_tab)) {

if (!empty($aggiungi_bene)) {
if ($priv_ins_beni != "n") {
if (!strcmp((string) $n_quantita_min_predef,"")) $n_quantita_min_predef = 0;
if (controlla_num_pos($n_quantita_min_predef) == "SI") {
if (!strcmp((string) $n_quantita,"")) $n_quantita = $n_quantita_min_predef;
...
if (strcmp((string) $prezzo_costo_agg,"") and controlla_soldi($prezzo_costo_agg) == "SI") {
...
} # fine if (strcmp((string) $prezzo_costo_agg,"") and controlla_soldi($prezzo_costo_agg) == "SI")
...
if (strcmp((string) $n_quant,"") and controlla_num_pos($n_quant) == "SI") {
...
} # fine if (strcmp((string) $n_quant,"") and controlla_num_pos($n_quant) == "SI")
...
} # fine else if (!empty($aggiungi_bene))

} # fine if (!empty($manda_form_tab))


if (!empty($ricarica) and $id_bene and $priv_mod_beni != "n") {
...
} # fine if (!empty($ricarica) and $id_bene and $priv_mod_beni != "n")


if (!empty($elimina) and $id_bene and $priv_ins_beni != "n") {
$id_bene = aggslashdb($id_bene);
if ($bene_permesso[$id_bene]) {
$tabelle_lock = array("$tablerelinventario");
$altre_tab_lock = array("$tablebeniinventario");
...
} # fine if (!empty($elimina) and $id_bene and $priv_ins_beni != "n")

} # fine if (!empty($aggiorna_inventario) and $priv_vedi_beni_inv != "n")
...
<input type=\"hidden\" name=\"id_sessione\" value=\"$id_sessione\">
<input type=\"hidden\" name=\"origine\" value=\"".htmlspecialchars($origine)."\">
<input type=\"hidden\" name=\"origine_vecchia\" value=\"".htmlspecialchars(fixstr($origine_vecchia))."\">
<input type=\"hidden\" name=\"tipo_tabella\" value=\"".htmlspecialchars($tipo_tabella)."\">
<input type=\"hidden\" name=\"idmag\" value=\"".htmlspecialchars($idmag_orig)."\">
<input type=\"hidden\" name=\"aggiorna_inventario\" value=\"SI\">
<input type=\"hidden\" name=\"manda_form_tab\" value=\"SI\">
<div class=\"tab_cont\">
...
<td class=\"t1top\" style=\"line-height: 70%\"><small><small>".mex("Quantità minima predefinita",$pag)."</small></small></td>
<td class=\"t1top\"><small>".mex("Quantità attuale",$pag)."</small></td>";
if ($priv_mod_beni != "n") {
echo "<td class=\"t1top\"><select id=\"ti_nq\" class=\"smallsel85\" name=\"tipo_nuova_quantita\" onchange=\"simb_tipo_quant();\">
...
if ($attiva_checkin == "SI") echo "<td class=\"t1top\" style=\"line-height: 70%\"><small><small>".str_replace("_","&nbsp;",mex("richiesto_per registrare entrata",$pag))."</small></small></td>";
...
echo "".mex("crea un <em>costo aggiuntivo</em>",$pag)."<br> ".mex("per il punto vendita",$pag).".</label></small></td>";
...
if ($num_beni_in_mag > 40 or ($mobile_device and $num_beni_in_mag > 20)) {
$id_modi = " id=\"modi\"";
$a_bottom = "<a id=\"bott\" name=\"bottom\"><span style=\"display: none;\">".mex("Vai a fondo pagina",$pag)."</span></a>";
} # fine if ($num_beni_in_mag > 40 or ($mobile_device and $num_beni_in_mag > 20))
else {
$id_modi = "";
$a_bottom = "";
} # fine else if ($num_beni_in_mag > 15)
echo "</table></div><br><div style=\"text-align: center;\">";
...
<input type=\"hidden\" name=\"id_sessione\" value=\"$id_sessione\">
<input type=\"hidden\" name=\"tipo_tabella\" value=\"$tipo_tabella\">";
if (!empty($origine_vecchia)) echo "<input type=\"hidden\" name=\"origine\" value=\"".htmlspecialchars($origine_vecchia)."\">";
echo "<button class=\"gobk\" type=\"submit\"><div>".mex("Torna indietro",$pag)."</div></button>
</div></form><br>$a_bottom</div>";
