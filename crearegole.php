...
$pag = "crearegole.php";
$titolo = "IdealVilla: Creer des regles";
...
<input type=\"hidden\" name=\"inizioperiodo\" value=\"".htmlspecialchars(fixstr($inizioperiodo))."\">
<input type=\"hidden\" name=\"fineperiodo\" value=\"".htmlspecialchars(fixstr($fineperiodo))."\">
...
if (!$inizioperiodo or !$fineperiodo or !$tipotariffa or substr($tipotariffa,0,7) != "tariffa" or controlla_num_pos(substr($tipotariffa,7)) == "NO" or ($attiva_tariffe_consentite != "n" and !isset($tariffe_consentite_vett[substr($tipotariffa,7)]))) {
echo mex("Non sono stati inseriti tutti i dati necessari",$pag)."!<br>";
...
if (!empty($regola_3) and $priv_mod_reg2 == "s") {
if (substr($tipotariffa,0,7) != "tariffa" or controlla_num_pos(substr($tipotariffa,7)) == "NO" or ($attiva_tariffe_consentite != "n" and !isset($tariffe_consentite_vett[substr($tipotariffa,7)]))) {
$inserire = "NO";
...
} # fine if (!empty($regola_3) and $priv_mod_reg2 == "s")
...
$origine = "tab_tariffe.php?numtariffa1=".htmlspecialchars(substr($tipotariffa_regola2.fixset($tipotariffa_regola3),7));
...
$inizio_select = "";
$fine_select = "";
$motivazione_orig = htmlspecialchars(fixstr($motivazione));
